...
- **`compose-plugin-check.json`** — a read-only, no-upload smoke that a freshly-provisioned VM has the
  docker prerequisites vmlease's cloud-init installs (Docker, the `docker compose` v2 plugin, buildx,
  ...
  ```sh
  uv run vmlease run --battery examples/compose-plugin-check.json \
